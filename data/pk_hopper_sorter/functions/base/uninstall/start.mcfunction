...
# - Append Waystones
data modify storage pk.common:data Uninstall.Array.Search append from storage pk.ho_so:data Blocks.HopperSorters[]

# Logs
tellraw @a [{"text": "Started uninstalling process for ","color": "yellow"},{"text": "KawaMood's Hopper Sorters","color": "aqua","bold": true},{"text":". It might take some seconds...","color": "yellow"}]

# Recursive function
execute if data storage pk.common:data Uninstall.Array.Search[{}] run function pk_hopper_sorter:base/uninstall/recursive
execute unless data storage pk.common:data Uninstall.Array.Search[{}] run function pk_hopper_sorter:base/uninstall/stop